...
def test_isolated_target_application_mounts_target_catalog_and_compat_routes() -> None:
    ...
    # Connect Apps was removed from this fork: DroppedNeedle no longer acts as a
    # Subsonic/Jellyfin server. Asserted negatively so a re-introduction is caught.
    assert "api.compat.subsonic.router" not in route_modules
    assert "api.compat.jellyfin.router" not in route_modules
    assert {
